...
# TA-algorithm to generate G-optimal OofA mixture orthogonal block design (K-MODEL)
import numpy as np
import pandas as pd
...
block1 = np.array([
    [0.239, 0.761, 0.000,  1,  0,  0],
    [0.239, 0.761, 0.000, -1,  0,  0],
    [0.761, 0.000, 0.239,  0, -1,  0],
    [0.761, 0.000, 0.239,  0,  1,  0],
    [0.000, 0.239, 0.761,  0,  0,  1],
    [0.000, 0.239, 0.761,  0,  0, -1],
...
block2 = np.array([
    [0.239, 0.000, 0.761,  0,  1,  0],
    [0.239, 0.000, 0.761,  0, -1,  0],
    [0.761, 0.239, 0.000, -1,  0,  0],
    [0.761, 0.239, 0.000,  1,  0,  0],
    [0.000, 0.761, 0.239,  0,  0, -1],
    [0.000, 0.761, 0.239,  0,  0,  1],
    [0.333, 0.333, 0.334,  1,  1,  1],
    [0.333, 0.333, 0.334,  1,  1, -1],
    [0.333, 0.333, 0.334,  1, -1, -1],
...
# For generating moel matrix
def generate_model_matrix(design):
    x1, x2, x3, z12, z13, z23 = design.T
    return np.column_stack([
        x1**2, x2**2, x3**2, z12, z13, z23,
        x1*x2, x1*x3, x2*x3,
        x1*z12, x3*z13, x2*z23
    ])

# Check orthogonal blocking conditions under K-model
def check_strict_orthogonality(b1, b2, atol=1e-3):

   
    for i in range(3):  # x1, x2, x3
        if not np.isclose(np.sum(b1[:, i]**2),
                          np.sum(b2[:, i]**2),
                          atol=atol):
            return False

   
    x_interactions = [(0,1), (0,2), (1,2)]
    for i, j in x_interactions:
        if not np.isclose(np.sum(b1[:, i] * b1[:, j]),
                          np.sum(b2[:, i] * b2[:, j]),
                          atol=atol):
            return False

    for i in range(3, 6):  # z12, z13, z23
        if not np.isclose(np.sum(b1[:, i]),
                          np.sum(b2[:, i]),
                          atol=atol):
            return False
